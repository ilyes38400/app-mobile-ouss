...
abstract class UserStoreBase with Store {
  ...
  @observable
  bool shouldReloadWorkoutLogs = false;
  ...
  Future<void> loadUserProfile() async {
    ...
    try {
      final userProfile = await loadUserProfileApi();

      if (userProfile.status == true) {
        isSubscribe = userProfile.isSubscribe ?? 0;
        print('Profil chargé: isSubscribe=$isSubscribe');
      } else {
        print('loadUserProfile: status=false -> ');
        // isSubscribe = 0;
      }
    } catch (e, stack) {
      // On logge aussi la stacktrace pour mieux comprendre
      print('Erreur loadUserProfile: $e');
      print(stack);
    }
  }
  ...
  @action
  Future<void> setIdealWeight(String val) async {
    idealWeight = val;
