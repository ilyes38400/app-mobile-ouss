...
class _BodyPartComponentState extends State<BodyPartComponent> {
  ...
  Widget build(BuildContext context) {
    double width = widget.isGrid == true ? (context.width() - 64) / 3 : context.width() * 0.28;
    double height = widget.isGrid == true ? (context.width() - 64) / 3 : context.width() * 0.28;

    return SizedBox(
      width: widget.isFilter == true ? context.width() * 0.24 : width,
      child: Column(
        children: [
          cachedImage(widget.bodyPartModel!.bodypartImage.validate(), fit: BoxFit.fill, width: width, height: height).cornerRadiusWithClipRRect(200),
          6.height,
