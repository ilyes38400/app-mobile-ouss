...
class Workout {
  ...
  start() {
    if (_soundEnabled) flutterTts.awaitSpeakCompletion(true);
    if (_step == WorkoutState.initial) {
      _step = WorkoutState.starting;

      if (_soundEnabled) flutterTts.speak("three");
  ...
  _tick(Timer timer) {
    ...
    if (_timeLeft.inSeconds == 1) {
      ...
      if (_step == WorkoutState.starting) {
        if (_soundEnabled) flutterTts.speak(_timeLeft.inSeconds.toString());
      } else {
        if (_step != WorkoutState.resting) {
          if (_timeLeft.inSeconds == 10) {
            if (_soundEnabled) flutterTts.speak("Ten second remaining");
          }
        }
        if (_timeLeft.inSeconds == 3) {
          // _playSound(_settings.countdownPip);
          if (_step == WorkoutState.resting) {
            if (_soundEnabled) flutterTts.speak("Three second remaining for rest");
          } else {
            if (_soundEnabled) flutterTts.speak("Three second remaining");
          }
        }
      }
    }
  ...
  _nextStep() {
    if (_step == WorkoutState.exercising) {
      ...
      if (_soundEnabled) flutterTts.speak("Go");
  ...
  _startRest() {
    ...
    if (_soundEnabled) flutterTts.speak('${_timeLeft.inSeconds.toString()} second rest up next ${_config.exerciseTime![_rep]} ${_config.status}');
  }
  ...
  _startBreak() {
    ...
    if (_soundEnabled) flutterTts.speak("break");
  ...
  _finish() {
    ...
    if (_soundEnabled) flutterTts.speak("Exercise Complete");
